...
def selection(D, assign):
    """
    Selección de variable no asignada de forma aleatoria. La función recibe el diccionario de dominio de las distintas variables "D" y un diccionario con las variables ya asignadas "assign".
...
def consistent(var, value, T, assign, R2, M):
    """
...
def backtrack(T, assign, D, R2, M):
    ...
    """
    if (len(assign) == len(T)): # Condición de salida
        return assign

    var = selection(D, assign) # Falta selecciónar solo entre las no asignadas [SOLUCIONADO?]

    for i in D[var]:
        if (consistent(var, i, T, assign, R2, M)):
            assign[var] = i
            ans = backtrack(T, assign, D, R2, M)
            if (ans != False):
                return ans
            assign.pop(var)
    return False
